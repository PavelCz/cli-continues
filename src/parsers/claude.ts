...
export function relocateClaudeSessionForCwd(session: UnifiedSession): void {
  ...
  const targetDir = path.join(CLAUDE_PROJECTS_DIR, claudeProjectSlugFromCwd(session.cwd));
  if (path.resolve(path.dirname(original)) === path.resolve(targetDir)) return;

  const fileName = path.basename(original);
  const backupDir = path.join(homeDir(), '.continues', 'backups', 'claude', path.basename(path.dirname(original)));
  ...
  console.log(`Moved Claude session file to ${targetDir} (backup: ${backupDir})`);
}
