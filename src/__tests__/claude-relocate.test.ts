...
describe('relocateClaudeSessionForCwd', () => {
  it('moves the session file into the launch cwd slug folder and keeps a backup', async () => {
    ...
    const movedPath = path.join(configDir, 'projects', '-new-launch-dir', 'sid-1234.jsonl');
    const backupPath = path.join(testState.home, '.continues', 'backups', 'claude', '-old-project', 'sid-1234.jsonl');
  ...
  it('invalidates the cached session index after a move', async () => {
    ...
    fs.writeFileSync(path.join(continuesDir, 'sessions.claude.jsonl'), 'stale\n');
    ...
    expect(fs.existsSync(path.join(continuesDir, 'sessions.jsonl'))).toBe(false);
    expect(fs.existsSync(path.join(continuesDir, 'sessions.claude.jsonl'))).toBe(false);
  });
  ...
  it('is wired as the claude adapter prepareNativeResume hook', async () => {
    ...
    const { relocateClaudeSessionForCwd } = await import('../parsers/claude.js');
    expect(typeof relocateClaudeSessionForCwd).toBe('function');
    expect(adapters.claude.prepareNativeResume).toBe(relocateClaudeSessionForCwd);
  });
});
