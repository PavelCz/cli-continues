...
describe('interactivePick native resume', () => {
  ...
  it('pins the current directory group first even when other groups are newer', async () => {
    ...
    testState.getAllSessions.mockResolvedValue([newerElsewhere, currentDirSession]);
    testState.selectTargetTool.mockResolvedValue('codex');
    let selectCall = 0;
    testState.select.mockImplementation(async (config: { options: Array<{ value: Record<string, unknown> }> }) => {
      selectCall += 1;
      if (selectCall === 1) {
        firstOptions = config.options;
        return config.options.find((option) => option.value.kind === 'directory' && option.value.cwd === process.cwd())
          ?.value;
      }
      if (selectCall === 2) {
        return config.options.find(
          (option) =>
            ...
            (option.value.session as UnifiedSession | undefined)?.id === 'current-dir-session',
        )?.value;
      }
  ...
  it('peeks at a session and returns to the action menu before resuming', async () => {
    ...
    testState.selectTargetTool.mockResolvedValue('codex');
    let selectCall = 0;
    testState.select.mockImplementation(async (config: { options: Array<{ value: unknown; label: string }> }) => {
      selectCall += 1;
