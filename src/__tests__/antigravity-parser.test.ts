...
function createCliConversationDb(
  dbPath: string,
  rows: Array<{ idx: number; stepType: number; payload: string | Buffer }>,
): void {
  const sqliteModule = require('node:sqlite') as {
    DatabaseSync: new (database: string) => SqliteDatabase;
  };
  ...
  try {
    ...
    for (const row of rows) {
      insert.run(
        row.idx,
        row.stepType,
        typeof row.payload === 'string' ? Buffer.from(row.payload, 'utf8') : row.payload,
      );
    }
  } finally {
    db.close();
...
describe('Antigravity parser', () => {
  ...
  it('uses per-conversation workspace metadata for older CLI threads and preserves protobuf messages', async () => {
    const root = makeRoot();
    const id = 'eeeeeeee-bbbb-4ccc-8ddd-eeeeeeeeeeee';
    vi.stubEnv('ANTIGRAVITY_CLI_HOME', root);
    const userText = `fix session ${id}\nこんにちは`;
    const assistantText = 'ok\n```ts\nconst x = 1;\n```';
    // Field paths observed in local CLI steps: user 19/2, planner 20/1,
    // checkpoint 30/4 (title), metadata 5/1 (timestamp).
    createCliConversationDb(path.join(root, 'conversations', `${id}.db`), [
      {
        idx: 0,
        stepType: 14,
        payload: Buffer.concat([
          fieldBytes(5, fieldBytes(1, timestampMessage(new Date('2026-06-01T00:00:00Z')))),
          fieldBytes(19, fieldBytes(2, Buffer.from(userText))),
        ]),
      },
      { idx: 1, stepType: 23, payload: fieldBytes(30, fieldBytes(4, Buffer.from('Checkpoint title'))) },
      { idx: 2, stepType: 15, payload: fieldBytes(20, fieldBytes(1, Buffer.from(assistantText))) },
    ]);
    writeFile(
        ...
        conversations: {
          [id]: {
            summary: {
              ...
              Preview: 'Preview',
              WorkspaceURIs: ['file:///home/user/my%20project'],
    ...
    expect(session.summary).toBe('Custom CLI title');
    expect(session.updatedAt.toISOString()).toBe('2026-06-02T00:00:00.000Z');
    const context = await extractAntigravityContext(session);
    expect(context.recentMessages.map(({ role, content }) => ({ role, content }))).toEqual([
      { role: 'user', content: userText },
      { role: 'assistant', content: assistantText },
    ]);
    expect(context.recentMessages[0].timestamp?.toISOString()).toBe('2026-06-01T00:00:00.000Z');
    expect(spawnMock).not.toHaveBeenCalled();
  });

  it('reads committed WAL steps and SQLite summary metadata without changing source storage', async () => {
    const root = makeRoot();
    vi.stubEnv('ANTIGRAVITY_CLI_HOME', root);
    const id = 'eeeeeeee-bbbb-4ccc-8ddd-eeeeeeeeeeee';
    const dbPath = path.join(root, 'conversations', `${id}.db`);
    createCliConversationDb(dbPath, []);
    const { DatabaseSync } = require('node:sqlite') as { DatabaseSync: new (file: string) => SqliteDatabase };
    const writer = new DatabaseSync(dbPath);
    const summary = new DatabaseSync(path.join(root, 'conversation_summaries.db'));
    try {
      writer.exec('PRAGMA journal_mode=WAL; PRAGMA wal_autocheckpoint=0');
      writer
        .prepare('INSERT INTO steps (idx, step_type, step_payload) VALUES (?, ?, ?)')
        .run(0, 14, fieldBytes(19, fieldBytes(2, Buffer.from('yes'))));
      summary.exec(
        'CREATE TABLE conversation_summaries (conversation_id TEXT, title TEXT, preview TEXT, workspace_uris TEXT, last_modified_time TEXT)',
      );
      summary
        .prepare('INSERT INTO conversation_summaries VALUES (?, ?, ?, ?, ?)')
        .run(id, 'Saved title', 'Preview', JSON.stringify(['file:///home/user/project']), '2026-09-01T00:00:00Z');
      const files = fs.readdirSync(path.dirname(dbPath)).sort();
      const before = files.map((name) => fs.readFileSync(path.join(path.dirname(dbPath), name)));
      const [session] = await parseAntigravitySessions();
      expect(session).toMatchObject({ summary: 'Saved title', cwd: '/home/user/project', lines: 1 });
      const context = await extractAntigravityContext(session);
      expect(context.recentMessages.map((message) => message.content)).toEqual(['yes']);
      expect(fs.readdirSync(path.dirname(dbPath)).sort()).toEqual(files);
      expect(files.map((name) => fs.readFileSync(path.join(path.dirname(dbPath), name)))).toEqual(before);
    } finally {
      writer.close();
      summary.close();
    }
  ...
  it('does not launch the IDE for an empty or unreadable CLI database', async () => {
    const root = makeRoot();
    vi.stubEnv('ANTIGRAVITY_CLI_HOME', root);
    vi.stubEnv('ANTIGRAVITY_DISABLE_RPC', '0');
